...
// Runtime data
var LOAD_OK = { schedule:false, ucty:false, uklid_log:false, penalizace:false, pracovni_dny:false, workers:false, trzby:false, naklady:false, businesses:false, venues:false, admin_venues:false, stanoviste:false, worker_venues:false };
var SCHEDULE = [];
var UCTY_POLOZKY = [];
var UKLID_LOG = [];
var PENALIZACE = [];
...
function getVydelekVenue(workerId, venueId) {
  ...
  SCHEDULE.forEach(function(s){
    if (s.workerId!==workerId || s.venue_id!==venueId || s.datum>today) return;
    if (seen[s.datum]) return; seen[s.datum]=true;
    var pd = getPracovniDen(workerId, s.datum);
    if (pd && pd.prichod && pd.odchod) hodiny += calcHodiny(pd.prichod, pd.odchod);
  });
